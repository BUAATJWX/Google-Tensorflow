# ***
# ******************************************************************************
# * @filename: The process of train and hyperparameter of CNN
# * @author  : tjwx
# * @version :
# * @date    : 2018.09.27
...
def train(mnist):
	...
	x = tf.placeholder(tf.float32, shape=[
		...
		mnist_inference2.NUM_CHANNELS],
					   name='x-input')
	y_ = tf.placeholder(tf.float32, shape=[None, mnist_inference2.OUTPUT_NODE], name='y-input')

	# define regularization and forward propagation
	global_step = tf.Variable(0, trainable=False)
	regularizer = tf.contrib.layers.l2_regularizer(regularization_rate)
	y = mnist_inference2.inference(x,False,regularizer,False)
